...
SELECT
    ...
    SUM(
        CASE
            WHEN line_item_line_item_type = 'RIFee' THEN line_item_normalization_factor
            ELSE 0
        END
    ) AS lineitem_normalizationfactor,
    SUM(line_item_normalized_usage_amount) AS lineitem_normalizedusageamount,
            ...
            WHEN line_item_line_item_type = 'RIFee' THEN (
                ...
                CASE
                    ...
                    ELSE (
                        reservation_amortized_upfront_fee_for_billing_period /
                        (
                            DATEDIFF(hour, line_item_usage_start_date, DATEADD(second, 2, line_item_usage_end_date)) * reservation_numberofreservations
                        )
                    )
                END
            )
...
WHERE
    extract(month from m.line_item_usage_start_date) = #startmonth
   and extract(year from m.line_item_usage_start_date) = #startyear and
    -- AND m.product_productname = 'Amazon Elastic Compute Cloud' and
    m.line_item_line_item_type IN ('RIFee', 'DiscountedUsage', 'VDiscountedUsage')
    AND m.line_item_line_item_type NOT IN ('Credit', 'Tax', 'Refund')
GROUP BY ALL;
...
CREATE TABLE IF NOT EXISTS ck_analytics_application_ri_wastage_hourly (
    RESERVATION_ARN VARCHAR,
    PAYER_NAME VARCHAR,
    BILL_PAYERACCOUNTID VARCHAR,
    USAGEACCOUNTID VARCHAR,
    ACCOUNT_TYPE VARCHAR,
    CUSTOMER_NAME VARCHAR,
    REGION VARCHAR,
    PRODUCT_NAME VARCHAR,
    OPERATINGSYSTEM_ENGINE VARCHAR,
    INSTANCETYPE_FAMILY VARCHAR,
    SAVINGSPLAN_TYPE VARCHAR,
    LINEITEM_USAGESTARTDATE TIMESTAMP,
    TOTAL_NU DECIMAL(30,8),
    USED_NU DECIMAL(30,8),
    UNUSED_NU DECIMAL(30,8),
    WASTAGE_PERCENTAGE DECIMAL(30,8),
    OFFERING_TYPE VARCHAR,
    RESERVATION_RESERVATIONARN VARCHAR,
    RESERVEDHOURS DECIMAL(30,8),
    USED_HOURS DECIMAL(30,8),
    UNUSED_HOURS DECIMAL(30,8),
    WASTAGE_COST DECIMAL(30,8),
    RI_UTILIZATION DECIMAL(30,8),
    MYCLOUD_STARTMONTH INTEGER,
    MYCLOUD_STARTYEAR INTEGER,
    INSTANCETYPE VARCHAR,
    NFAPPLIED DECIMAL(30,8),
    QUANTITY DECIMAL(30,8),
    RI_ECOST DECIMAL(30,8)
);
...
RI_ECOST
from ec2_ri_utilization_report_hourly_temp a
-- left join PAYERS_MAX_DATE b on a.BILL_PAYERACCOUNTID = b.BILL_PAYERACCOUNTID and
 -- b.product_name = 'Amazon Elastic Compute Cloud'
where 
-- a.STARTDATE <= b.max_date and 
reservedHours <> 0;
